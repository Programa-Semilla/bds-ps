...
<!-- SPECKIT START -->
Active plan: `specs/026-input-masks/plan.md` — Structured-field input masks. Generalize the hand-rolled `wwwroot/js/input-masks.js` into a data-driven, event-delegated mask **registry** (covers AJAX-injected supplier partials; add-a-field = one entry + `data-mask` attr). Mask catalogue: `email`, `phone-cr`, `cedula` (`0-0000-0000`), `cedula-jur` (`3-000-000000`), `dimex` (11–12 digits), `nite`, `pasaporte`. New domain `IdentificationType` enum (`: byte`) + `Identification` value object (per-type `[GeneratedRegex]`, canonicalize+validate — mirrors `CurrencyCode`/`PublicCode`) owning the type↔shape invariant. Persist on `dbo.Applicants.IdentificationType` + `dbo.Suppliers.IdentificationType` (TINYINT NULL, dacpac) — **person legal ID lives on `Applicant`, NOT `AspNetUsers`**. Editable type selector (rebinds the field's mask) on Register / admin user create+edit / supplier add; **read-only** display on Profile (identity is admin-managed). Server validation via `IdentificationFormatAttribute` delegating to the VO (errors aggregated, es-CR). Canonical stored form = **hyphenated**; `Supplier.NormalizeLegalId` extended to canonicalize so supplier lookup is hyphenation-tolerant. Pre-production → seeds (`IdentityConfiguration.cs` demo cédulas → `1-0001-000X`) + E2E (`RegisterUserAsync`, page objects) updated to valid canonical values, no backfill. Completes spec-021 FR-013 (mask infra existed but was wired to a view with no maskable fields).
<!-- SPECKIT END -->
