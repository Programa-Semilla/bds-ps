...
<!-- SPECKIT START -->
Active plan: **033-user-invite-email** — planned (`specs/033-user-invite-email/plan.md`). Replace the spec-032 admin-typed temp password with an emailed 72h single-use **set-password invitation**: admin create drops the password field (`CreateAsync(user)` no-password, `MustChangePassword=false`); the controller issues a 72h reset/invite token (reuse the password-reset flow — add `Ttl` + `InvalidatePriorUnused` to the issue command, `InvalidateUnusedAsync` to the store) + composes the `/Account/ResetPassword` link + direct-sends an es-CR invite email (`ForgotPasswordEmail` pattern, not the outbox) + renders an "Invitación enviada" confirmation with a copyable link (FR-008 fallback); **resend** action supersedes the prior link. **No schema change, no new deps.** E2E ripple: drop the create-form password fill, rewrite the two obsolete first-login change-password tests to the invite flow; `SeedUser` keeps password bootstrap. Next: `/speckit-tasks`.
