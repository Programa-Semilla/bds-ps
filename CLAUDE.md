...
<!-- SPECKIT START -->
Active plan: `specs/025-supplier-location-cascade/plan.md` — Supplier branch location cascade (Provincia → Cantón → Distrito). Finishes spec-021 FR-014 (the never-wired Province→Cantón cascade partial) and adds a 3rd level: new `dbo.Districts` catalog table + MERGE seed mirroring `dbo.Cantons` (488 distritos, `'PP_CC_DD'` codes), `District` domain entity, `SupplierBranches.DistrictId` FK, `GET /api/districts?cantonId=` endpoint, data-driven `location-cascade.js` (generalized from `province-canton-cascade.js`), 3-`<select>` `_LocationCascade.cshtml` partial wired into applicant new-supplier (`_LookupEmpty`), applicant new-branch (`_BranchPicker`), and admin branch-edit (`Detail.cshtml`). All-3-required (server-validated + aggregated); `SupplierBranch.SetLocation` extended to a 3rd tier; legacy `Province` string kept as composed display value. Verified: existing 84-cantón catalog already includes Puerto Jiménez `06_13` + Monteverde `06_12` (Golfito `06_07` = 3 distritos). Deviation flagged: domain allows province+cantón without distrito (forms require all 3) to keep the orphaned inline `CreateSupplierBranchHandler` path compiling.
<!-- SPECKIT END -->
