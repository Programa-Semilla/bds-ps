...
public class AdminSidebarGroupingTests : AuthenticatedTestBase
{
    ...
    public async Task Admin_SeesSectionHeaderAndAllSubEntries()
    {
        ...
        // All seven admin sub-entry slugs must be present (FR-016).
        var slugs = new[] { "users", "groups", "suppliers", "reports", "currencies", "exchange-rates", "legacy-quotations" };
    ...
    public async Task Applicant_DoesNotSeeAdminSection()
    {
        ...
        foreach (var slug in new[] { "users", "groups", "suppliers", "reports", "currencies", "exchange-rates", "legacy-quotations" })
        {
