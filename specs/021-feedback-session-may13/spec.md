...
- **FR-017**: Submit MUST be disabled until the Application has ≥ 1 Item, Impact defined, and every required field complete. Submit MUST route to `/review` showing items / suppliers / totals / Impact with an explicit *"Confirmar y enviar"* button before final send. The `/review` total ("Total estimado (CRC)") renders BEFORE any reviewer has selected a supplier per item, so it MUST be the **sum of the cheapest converted-CRC quotation of each item** ("menor cotización por ítem") — NOT the sum of every competing quotation, which would N-count each item. Items hold ≥ `MinimumQuotationsPerItem` competing offers and only one is ever funded; the estimate is therefore a lower bound. Legacy-flagged quotations (no converted amount) are excluded. *(Evolved 2026-05-25: the original implementation summed all quotations, producing a total no eventual purchase could match; the formula is now centralized in `ApplicationCurrencyTotal.ComputeCheapestEstimate`, sibling to the selected-supplier `Compute` used post-selection on Details / dashboard / reviewer-queue.)*
...
- **FR-029**: Public CTA MUST read *"¿Listo para acelerar tu negocio?"* with button *"Iniciar acompañamiento"*. The word *"financiamiento"* MUST be removed from every applicant-facing rendered surface; the *Funding Agreement* PDF MAY retain the legal term. *(Evolved 2026-05-25: the convenio panel's `DisabledReason` on `/Application/Details` rendered the raw English domain precondition strings from `CanGenerate/CanRegenerateFundingAgreement` (e.g. "Review is still in progress."). Per spec 012 FR-014 / NFR-001 the Application layer stays English; the Web `UserFacingErrorTranslator.TranslateAgreementDisabledReason` now maps the closed set of those strings to es-CR — phrased around "convenio", never "financiamiento" — applied at the `MapToViewModel` boundary in `FundingAgreementController`.)*
- **FR-030**: Welcome greeting MUST render *"Hola, {{Nombre}}"*. The string *"Bienvenido/a"* MUST NOT appear in any rendered applicant-facing surface. *(Evolved 2026-05-25: the applicant home dashboard leaked English on three surfaces — the per-application stage pill + mini-journey labels (`StageMappingProvider` catalogue: "Draft"/"Submitted"/"Under Review"/…), and the "Actividad reciente" timeline titles (`ApplicantDashboardProjection.PrettyAction`: "Application created"/"Application sent"/…). Both are Application-layer display copy (same category as the injected `IApplicantCopyProvider`, not error-codes) and are now es-CR — stage labels aligned with `StatusVisualMap`, "Funded" → "Fondos entregados" to avoid "financiamiento" (FR-029). The journey catalogue is shared, so reviewer/admin journey surfaces localize too. Two sibling leaks fixed in the same pass: the reviewer queue's "Actividad reciente" titles surfaced the raw `VersionHistory.Action` and its row CTA read "Review" / applicant fallback "Applicant" (`ReviewerQueueProjection`), and the admin Reports dashboard pipeline tiles rendered `ApplicationState.ToString()` (`AdminReportsController`). Activity-title copy is now centralized in `ActivityActionCopy.Title` (shared by applicant + reviewer); the reviewer CTA → "Revisar", fallback → "Solicitante"; pipeline tiles → `StatusVisualMap` Spanish labels. CSV export headers stay English by contract — they are machine-readable keys, not rendered copy. Follow-ups: `ActivityActionCopy` now covers the full written action vocabulary — `FlagEquivalence` ("no técnicamente equivalente") → "Equivalencia técnica actualizada" and `Withdrawn` → "Solicitud retirada" (both previously fell through to the raw code on the timeline; pinned by `ActivityActionCopyTests`). And the applicant timeline actor stopped rendering the raw Identity user-id GUID: `ApplicantDashboardProjection.ResolveActor` maps the applicant's own UserId → "usted" and any other actor → "Equipo del programa" (reviewer identities not exposed), enabled by adding `.Include(a => a.Applicant)` to `GetForApplicantDashboardAsync`. Also: `Item.FlagNotEquivalent()` no longer persists the English sentence "Rejected: quotations are not technically equivalent" as `ReviewComment` — the `IsNotTechnicallyEquivalent` boolean already carries the state, so `ReviewComment` is now null and the applicant Details page renders the localized "Las cotizaciones no son técnicamente equivalentes." from the flag (threaded through `ItemDto` → `ItemViewModel`). Reviewer free-text reject comments are untouched.)*
...
- **FR-033**: Pending-quotation tile MUST move from the admin dashboard to the reviewer dashboard with the same data source. *(Evolved 2026-05-25: the tile counted individual `dbo.Quotations` on Submitted applications, so an application with N competing quotes per item inflated the count (showed "2" for one application). It now counts **distinct Submitted applications** — units of pending review work — and is relabeled "Solicitudes pendientes" (slug `pending-applications`). `IReviewerDashboardProjection.CountPendingQuotationsAsync` → `CountPendingApplicationsAsync`. UnderReview stays excluded (it is reviewer-active, not pending pickup).)*
