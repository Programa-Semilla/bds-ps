...
- [ ] T009 [P] Create `IdentificationFormatAttribute` (resolves sibling `IdentificationType` property, delegates to `Identification.IsValid`, es-CR message "La identificación no tiene el formato de {tipo}.") in `src/FundingPlatform.Web/Validation/IdentificationFormatAttribute.cs`. (dep: T003)
...
- [ ] T024 [P] [US2] `AddSupplierViewModel`: add `SupplierIdentificationType` (jurídica/NITE) + `[IdentificationFormat]` on `SupplierLegalId` in `src/FundingPlatform.Web/ViewModels/AddSupplierViewModel.cs`. (dep: T009)
