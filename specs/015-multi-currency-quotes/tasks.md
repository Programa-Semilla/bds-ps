...
- [ ] T903 Ran the full E2E suite (`dotnet test tests/FundingPlatform.Tests.E2E --nologo`) — **NOT GREEN**: 90 passed / 47 failed / 0 skipped / 137 total in 22m35s. Failures cluster on `Locator("a:has-text('Agregar proveedor')")` 30s timeout inside legacy `SupplierSelectionTests.SetupSubmittedApplicationAsync`, `TechnicalEquivalenceTests.SetupSubmittedApplicationAsync`, `SupplierCatalogTests.US3_NewLegalId_LandsOnNewSupplierForm_AndCreatesDraft`. Spec-015 own E2E (US1–US6) all green per per-phase runs. Constitutional delivery gate (Constitution III + project memory "personally-executed green E2E") **not yet met**; root-cause is legacy supplier-add flow that may have been disturbed by Phase 6 view changes — needs investigation
