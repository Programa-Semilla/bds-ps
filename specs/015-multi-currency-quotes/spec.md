...
- **Concurrent admin edits**: Two admins try to create a new rate for the same currency pair within the same effective millisecond. Last-writer-wins is acceptable as long as both attempts are recorded in the audit log; the rejected one returns a duplicate-timestamp error. (Effective timestamps are stored at millisecond precision, narrowing the collision window vs. second precision.)
- **USD disabled mid-edit**: An applicant has the quote form open with USD selected; an administrator disables USD in another session. On save, if the form's quote is **new** (no existing snapshot), save fails with the standard "currency not enabled" validation error and the user must switch to CRC or abandon the form. If the quote **already exists**, amount edits remain allowed (FR-017b) and re-apply the original snapshot — disabling USD never strands an existing quote.
- **PDF refusal UX**: When PDF generation refuses (FR-027), the agreement page MUST re-render with the inline error directly on the same view (no redirect, no `TempData`-only mechanism), so a hard reload still surfaces the error until the offending quote is fixed.
...
- **FR-017**: Each `Quotation` (one supplier price on one Item) carries a single currency + single amount. The codebase has no "line items within a quote"; line items are modeled as `Item` records on the parent `Application`, and each `Item` collects multiple single-price `Quotation` rows from different suppliers. References to "quote totals" and "request totals" map to: the selected supplier's `Quotation.ConvertedCrcAmount` per Item, summed across the Items of an Application.
- **FR-017a**: A `Quotation`'s currency MAY be changed only via an explicit re-conversion action that **clears the existing rate snapshot and applies a new one** using the latest rate at the moment of change. Silent in-place currency edits without re-snapshotting are forbidden. Users with edit access who genuinely need a different currency follow the explicit re-conversion path (or delete the quotation and recreate it).
- **FR-017b**: Editing the **amount only** on an existing `Quotation` MUST re-apply the originally snapshotted rate rather than re-fetching the current latest rate. This guarantee MUST hold even if the quote's currency is currently disabled (FR-003).
