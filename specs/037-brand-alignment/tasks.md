...
- [ ] T020 [P] [US1] Visual check the PDF-preview iframe + applicant download CTA still render against the new chrome (no regression); note PDF brand asset itself is US5.
...
- [ ] T026 [P] [US3] Apply `_RowActionsMenu` to other admin tables with ≥2 row actions for consistency (enumerate: e.g. Suppliers, Groups, Currencies, Exchange Rates, Funds, Processes, Categories, ImpactTemplates lists); preserve their existing action testids/routes.
...
- [ ] T033 [P] [US6] Add a keyboard E2E (new or in an existing Brand test): the `⋯` kebab is reachable/operable by keyboard with a visible teal focus ring; status pills carry icon+text (color not sole signal) (FR-027/SC-009).
- [ ] T034 [P] [US6] Add responsive E2E checks: narrow viewport → filters wrap, tables horizontal-scroll, footer image scales, sidebar collapses to the icon-only logo (FR-024/SC-010).
