...
- **FR-024**: The system MUST ship six body variant partials, one per `NotificationEvent` enum value (`APPLICATION_SUBMITTED_APPLICANT`, `APPLICATION_SUBMITTED_REVIEWER`, `RETURNED_TO_APPLICANT`, `RESUBMITTED_BY_APPLICANT`, `APPLICATION_APPROVED`, `APPLICATION_REJECTED`). The participating-admin bucket reuses the SAME body partial that the bucket-priority-winning variant uses (no separate admin-flavored partials in v1 — `RETURNED_TO_APPLICANT`, `APPLICATION_APPROVED`, `APPLICATION_REJECTED` all use the applicant-variant body for admin recipients; `APPLICATION_SUBMITTED_REVIEWER` and `RESUBMITTED_BY_APPLICANT` use the reviewer-variant body for admin recipients). Each of the six variants MUST render an HTML body AND a plain-text fallback (12 `.cshtml` files in total — 6 HTML + 6 plain-text).
