...
- [x] T030 [P] [US1] Create `src/FundingPlatform.Infrastructure/Notifications/Resolvers/NotificationRecipientResolver.cs` — implements `INotificationRecipientResolver`. Bucket dedup per FR-012. Stage-group reviewer query reads via `AppDbContext` (UserGroupMembership + applicant's groups). Returns recipients in deterministic order (applicant first, then reviewers by user id ascending, then participating admins by user id ascending).
- [x] T031 [US1] Create `src/FundingPlatform.Infrastructure/Notifications/Resolvers/ParticipatingAdminPredicate.cs` — encapsulates the v1 predicate from `research.md` R-006 (current-Admin-role-only). Returns deterministic `IReadOnlyList<string>` of user ids. Documented v1 limitation re EC-002 / OQ-011.

### Workflow hook (Application Service)

- [x] T032 [US1] Edit `src/FundingPlatform.Application/Services/ApplicationService.cs` (the existing `SubmitApplicationAsync` method) to: (a) determine whether this submission is a first-time submit vs. resubmit by querying `VersionHistory` for any prior `Action="SendBack"` row on the same `ApplicationId`; (b) for FIRST-TIME submit only — enqueue TWO outbox rows in one transaction (`APPLICATION_SUBMITTED_REVIEWER` and `APPLICATION_SUBMITTED_APPLICANT`) via the injected `INotificationOutboxWriter`. Wrapped in an explicit `IWorkflowTransactionScope` so the workflow-state save + outbox-row save commit atomically (FR-001). Resubmit detection also wired here (US3 — sends `RESUBMITTED_BY_APPLICANT` instead of the two-row fan-out).

### Templates

- [x] T033 [P] [US1] Create `src/FundingPlatform.Web/Views/Emails/ApplicationSubmittedApplicant.cshtml` (HTML) — subject template `Recibimos tu solicitud — Solicitud #{Application.Id}`; body confirming receipt, CTA to `/Application/Details/{id}` composed from `Notifications:BaseUrl`.
- [x] T034 [P] [US1] Create `src/FundingPlatform.Web/Views/Emails/ApplicationSubmittedApplicant.text.cshtml` (plain text fallback).
- [x] T035 [P] [US1] Create `src/FundingPlatform.Web/Views/Emails/ApplicationSubmittedReviewer.cshtml` (HTML) — subject `Nueva solicitud para revisar: {ApplicantName}`; body announcing new work, CTA to `/Review/{id}`.
- [x] T036 [P] [US1] Create `src/FundingPlatform.Web/Views/Emails/ApplicationSubmittedReviewer.text.cshtml`.

### Worker (minimum viable dispatcher — single attempt, no retry yet)

- [x] T037 [US1] Create `src/FundingPlatform.Infrastructure/Notifications/Workers/EmailDispatchWorker.cs` — hosted `BackgroundService`. Poll loop. `ProcessBatchAsync` selects `Pending` rows ordered by `CreatedAt`, claims via `RowVersion` optimistic update (`Pending → Dispatching`), resolves recipients, checks `NotificationDelivery` for an existing dedup row before sending, calls `IEmailSender.SendAsync(...)`, writes `NotificationDelivery` + transitions outbox to `Done`. Worker already includes retry/backoff (T066) + permanent-failure dead-letter (T022) since they layer trivially over the base loop.
- [x] T038 [US1] Create `src/FundingPlatform.Infrastructure/Notifications/Providers/NoOpEmailSender.cs` — logs WARN, returns `EmailSendOutcome.Sent` with null `ProviderMessageId`. Used as default when no provider config is set (FR-015).
- [x] T039 [US1] Create `src/FundingPlatform.Infrastructure/Notifications/Providers/MailtrapSmtpEmailSender.cs` — MailKit v3 path. Builds `MimeMessage` with `multipart/alternative`, sends via `SmtpClient` to the configured (or Aspire-discovered) host/port. Maps SMTP outcomes per the table in `contracts/IEmailSender.md`. For US1, the smtp4dev sidecar accepts everything → always `Sent`.

### Unit tests

- [x] T040 [P] [US1] Create `tests/FundingPlatform.Tests.Unit/Notifications/NotificationTemplateBindingsTests.cs` — assert every `NotificationEvent` enum value has a binding row (subject template, html view name, text view name, variant key). Also covers subject interpolation + 78-char truncation + storage-string round-trip.
- [x] T041 [P] [US1] Create `tests/FundingPlatform.Tests.Unit/Notifications/RazorEmailRendererTests.cs` — source-level assertions on every template variant (all 6 events × HTML + text): no inline `<img>`, no `Capital Semilla` / `Forge`, no English-only marker phrases, rejection body never references reviewer-commentary tokens (NFR-003), text-only wordmark + sender display in layout, static soporte mailto in footer. Render-time assertions (real Razor output) move to the E2E suite where the Aspire stack is up.

### Integration test

- [x] T042 [US1] Create `tests/FundingPlatform.Tests.Integration/Notifications/OutboxTransactionalEnqueueTests.cs` — `EnqueueAsync_writes_outbox_row_on_SaveChanges` (writer defers persistence to caller's SaveChanges), `Submit_fails_writes_zero_outbox_rows` (rollback path — FR-001), `HasPriorSendBackAsync_detects_resubmit_path` (R-003).

### E2E tests (mandatory per Constitution §III)

- [x] T043 [US1] Create `tests/FundingPlatform.Tests.E2E/Notifications/ApplicationSubmittedNotificationsTests.cs` — full UI flow: register reviewer + applicant, applicant creates + submits a complete application (impact + 2 quotations), assert `MailCaptureClient.WaitForAsync` returns the applicant-variant + reviewer-variant messages with the expected subjects + CTA hrefs + sender display + no inline `<img>` + zero `Capital Semilla` / `Forge`. **Code complete and compiles**; live test run against the Aspire stack is gated by T086 (full E2E pass) since the suite is shared-fixture and runs in one pass.
...
- [x] T086 Run the **full** E2E suite locally: `dotnet test tests/FundingPlatform.Tests.E2E`. **Result 2026-05-12: Passed 206, Failed 0, Skipped 5, Total 211, Duration 7 m 4 s.** Five skipped are the intentional `Assert.Ignore` deferrals from US3 / US4 / US5 / US6 / US7 (T055, T060, T065, T071, T077) where writer / integration / source-level harness coverage is the load-bearing test; the deferral rationale is documented in each test's class summary. Delivery gate per memory `feedback_delivery_requires_e2e_green`: **MET**.
