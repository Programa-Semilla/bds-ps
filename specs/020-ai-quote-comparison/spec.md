...
- **FR-B2**: A PII redactor MUST process the assembled payload before any bytes leave the platform. The redactor invariant covers two surfaces:
  - **Field-level structured redaction** runs on the per-supplier DTO. The live domain (specs 011/013) carries one contact channel per actor, not a "personal vs business" split, so the MVP field set is: applicant legal id (`Applicant.LegalId`), applicant email (`Applicant.Email`), applicant phone (`Applicant.Phone`), supplier legal id (`Supplier.LegalId` — the FR-B2 "owner DNI" proxy), and the supplier-branch contact phone (`SupplierBranch.Phone` — the FR-B2 "owner personal phone" proxy). Each non-null value is replaced with `[REDACTED]` and the count surfaces on the audit row's `redactedFieldCounts` dictionary using the canonical key names (`applicantNationalId`, `applicantPersonalEmail`, `applicantPersonalPhone`, `supplierOwnerDni`, `supplierOwnerPhone`). When the domain later splits personal vs business channels, the assembler swaps the source field without touching the redactor.
  - **Pattern-level file-text redaction** runs on extracted PDF text (cédula regex, CR phone regex, email regex). DEFERRED in this MVP — the dep graph carries no PDF text-extraction library and adding one requires spec approval per CLAUDE.md (Anthropic.SDK was the only new dep approved). The orchestrator does NOT call `IPiiRedactor.RedactFileText` until a text-extraction seam exists; the redactor's pattern path stays unit-tested for the future seam. Field-level redaction (above) still scrubs the structured DB-side payload before any bytes leave.
- **FR-B3**: If a blob cannot be safely redacted (e.g., scanned image with no text layer), MVP default behaviour is to refuse the file with a clear "envíe un PDF con capa de texto" message. (See Assumption A-1 for the OCR alternative.) Today the `pii_redaction_failed` envelope is reachable only by calling `IPiiRedactor.RedactFileText` directly with empty text; once FR-B2's file-text seam is wired, the orchestrator will route image-only PDFs through this envelope.
...
- **A-12**: The supplier-quotation file storage category in spec 014 (`Storage:Categories:application-attachments` — `FileCategory.ApplicationAttachment`) already enforces upload-side size limits; this feature does not add another upload-side cap. The orchestrator carries a 25 MiB defense-in-depth ceiling per blob (FINDING-5) so a single oversized file cannot OOM the pipeline.
