...
- [ ] T031 [P] Coverage verification: grep confirms no `window.alert`/`window.confirm` remain in the affected flows (SC-001) and the banner `.alert` blocks are removed from both layouts (SC-002); capture the result.
