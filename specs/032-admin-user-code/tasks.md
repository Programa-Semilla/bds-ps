...
- **D-6 (US1, full-E2E collateral — found in the cross-cutting run):** The full E2E suite (320 passed / 5 failed / 5 skipped) surfaced 4 more tests that piggy-backed on the now-removed Register form (they navigate to `/Account/Register` directly, so the `RegisterPage` grep in D-2 missed them) plus 1 expected CSV change: (a) `AdminReportsApplicantsTests.Applicants_CsvExport_HasExpectedHeader` — updated the asserted header to include the new "User Code" column (FR-016); (b) `RequiredMarkerTests.ApplicantRegisterForm_RequiredFields_ShowMarker` — removed (the same file's `AdminUserCreateForm_RequiredMarked_OptionalNot` already covers required markers on a live form); (c) `HintTooltipTests.RegisterEmailField_HasInfoIcon...` — removed (the spec-027 email info-icon hint existed **only** on the register email field; the `_HintTooltip` mechanism stays covered on its other live pages); (d) `EnumExclusionTests.EnumDropdown_IsNotEnhanced_NoSearchBox` — relocated to the admin create form's `IdentificationType` enum select (same FR-009 assertion, surviving surface). **Pre-existing (not 032):** `FilterToolbarSearchTests.FundFilterLevel_AboveThreshold...` (the spec-031 admin Fund-cascade filter) fails on the branch base — it was added in `1b87745` (031, branch point), my commits never touched it / the cascade JS / its logic, and 031's delivery note omitted it from its green set (the "pre-existing admin cascade-filter WIP" per CLAUDE.md). Out of scope for 032.
